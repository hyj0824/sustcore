...
build-and-dbg: build + run_dbg
run: 启动 qemu
run_dbg: 使用 -s -S 启动 qemu
clean: rm -rf build
```

通常情况下，`make build`、`make run`、`make build-and-run` 和 `make build-and-dbg` 是最常用的目标。`make all` 会构建 `kernel-rv` 与 `kernel-la`，而不是运行 QEMU。
...
`arch` 字段决定 Makefile 围绕的架构，对于本地开发，决定了你 `make run` 跑起来的架构是哪个，也就是执行 `make build-and-run` 等命令时构建和运行的架构。
...
当前根目录 `Makefile` 中存在一个需要注意的地方：`build` 将 `make-initrd`、`build-mods` 和 `build-kernel` 写成同一个目标的多个先决条件。普通串行 `make` 通常会按书写顺序执行，但 Make 语义并不把同级先决条件之间的顺序作为强依赖表达；在并行构建或重构构建图时，`build-mods` 可能早于 `make-initrd` 访问 `$(path-initrd)`。预期的数据流是“先创建 initrd 暂存树，再将模块复制进去，最后链接内核并嵌入 initrd”，因此修改这里时应显式表达顺序依赖。
...
`run` 只启动当前 `$(path-kernel)` 指向的内核，不会先执行构建。需要“构建后运行”时使用 `make build-and-run`。`config.mk` 可以覆盖内存大小并追加 `qemu-attached-args`。
